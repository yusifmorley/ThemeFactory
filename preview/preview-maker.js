...
const templates = {
  [MINIMALISTIC_TEMPLATE]: fs.readFileSync(`static/svg/new-theme-preview.svg`, `utf8`),
  [REGULAR_TEMPLATE]: fs.readFileSync(`static/svg/theme-preview.svg`, `utf8`),
  [NEW_TEMPLATE]: fs.readFileSync(`static/svg/new-preview.svg`, `utf8`),
  [DESKTOP_TEMPLATE]: fs.readFileSync(`static/svg/tdesktop-preview.svg`, `utf8`),
};
const defaultThemeBuffer = fs.readFileSync(`static/classic-tdesktheme/classic.tdesktop-theme`);
...
for (let index = 0; index < WALLPAPERS_AMOUNT; index++) {
  const wallpaper = fs.readFileSync(`static/wallpapers/${index}.jpg`, `binary`);
