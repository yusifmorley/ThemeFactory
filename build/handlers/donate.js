...
const amountButtons = new grammy_1.InlineKeyboard()
    .text('$5', 'donate:5')
    .text('$10', 'donate:10')
    .row()
    .text('$20', 'donate:20')
...
const replyButtons = new grammy_1.InlineKeyboard().pay('Donate ❤️');
...
const getInvoice = (amount) => {
    ...
    return {
        ...
        provider_token: env_1.default.STRIPE_TOKEN,
        currency: 'USD',
        prices: [
            {
                label: 'Donate',
...
privateFilter.command('start', async (ctx, next) => {
    ...
    if (!env_1.default.STRIPE_TOKEN) {
        ...
        if (invoice.valid) {
            await ctx.replyWithInvoice(invoice.title, invoice.description, invoice.payload, invoice.provider_token, invoice.currency, invoice.prices, invoice.extra);
        }
        else {
        ...
        await ctx.reply(`Thanks for helping support @${ctx.me.username}! How much would you like to donate? (All amounts are in USD)`, { reply_markup: amountButtons });
    }
});
composer.command('donate', async (ctx) => {
    if (!env_1.default.STRIPE_TOKEN) {
        ...
        await ctx.reply(`Thanks for your interest in helping support @${ctx.me.username}! Please contact me in PM to donate.`, {
            reply_markup: new grammy_1.InlineKeyboard().url('Donate', `https://t.me/${ctx.me.username}?start=donate`),
        });
    }
});
composer.callbackQuery(/^donate:(\d+|custom)$/, async (ctx) => {
    ...
    if (amount === 'custom') {
        await ctx.editMessageText(`Thanks for helping support @${ctx.me.username}! Please specify the amount you want to donate (in USD).`);
        await ctx.answerCallbackQuery();
        return;
    }
    let _amount;
