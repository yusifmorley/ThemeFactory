"use strict";
var __importDefault = (this && this.__importDefault) || function (mod) {
    return (mod && mod.__esModule) ? mod : { "default": mod };
};
Object.defineProperty(exports, "__esModule", { value: true });
const promises_1 = __importDefault(require("fs/promises"));
const grammy_1 = require("grammy");
const env_1 = __importDefault(require("../env"));
const colors_1 = require("../utils/colors");
const keyboard_1 = require("../utils/keyboard");
const storage_1 = require("../utils/storage");
const theme_preview_1 = __importDefault(require("../utils/theme-preview"));
const themes_1 = require("../utils/themes");
const composer = new grammy_1.Composer();
composer.callbackQuery(/^cancel,(\d+)$/, async (ctx) => {
    if (Number(ctx.match[1]) === ctx.from.id) {
        await ctx.deleteMessage();
        await ctx.answerCallbackQuery();
        if (ctx.msg) {
            await storage_1.saveTheme(ctx, ctx.msg.message_id, null);
        }
    }
    else {
        await ctx.answerCallbackQuery({ text: ctx.i18n('not_your_theme') });
    }
});
const requireTheme = composer.filter((ctx) => {
    if (!ctx.msg) {
        return false;
    }
    return true;
});
requireTheme.on('callback_query', async (ctx, next) => {
    // themeid  即为 ctx.msg 里的 message_id
    const { message_id: themeId } = ctx.msg;
    //从数据库里查询记录
    const theme = await storage_1.getTheme(ctx, themeId);
    if (!theme) {
        await ctx.answerCallbackQuery({
            text: ctx.i18n('no_theme_found'),
            show_alert: true,
        });
        return;
    }
    //进行赋值
    ctx.theme = theme;
    await next();
});
//接受到了 callbackQuery 的default 命令
requireTheme.callbackQuery('default', async (ctx) => {
    ctx.theme.using = [
        { label: '1', color: ctx.theme.colors[0] },
        { label: '5', color: ctx.theme.colors[4] },
        { label: '4', color: ctx.theme.colors[3] },
    ];
    await storage_1.saveTheme(ctx, ctx.msg.message_id, ctx.theme);
    await ctx.editMessageCaption({
        caption: ctx.i18n('type_of_theme'),
        reply_markup: keyboard_1.getTypeKeyboard(ctx),
    });
});
//接受到了 callbackQuery 的 - 命令
requireTheme.callbackQuery('-', async (ctx) => {
    ctx.theme.using.pop();
    await storage_1.saveTheme(ctx, ctx.msg.message_id, ctx.theme);
    const { length } = ctx.theme.using;
    const keyboard = keyboard_1.getKeyboard(ctx, length > 0);
    await ctx.editMessageCaption({
        caption: ctx.i18n(`choose_color_${length + 1}`, {
            colors: colors_1.labelColors(ctx.theme.using).join(', '),
        }),
        reply_markup: keyboard,
    });
});
//接受到了 callbackQuery 的 - 命令
requireTheme.callbackQuery(['attheme', 'tgios-theme', 'tgx-theme'], async (ctx) => {
    const { photo, using } = ctx.theme;
    const name = themes_1.getThemeName(using[0].color, using[2].color);
    const theme = themes_1.createTheme({
        username: ctx.me.username,
        type: ctx.callbackQuery.data,
        name,
        image: await promises_1.default.readFile(photo),
        colors: using.map(c => c.color),
    });
    const caption = [
        `Made by @${ctx.me.username}`,
        `#theme ${colors_1.labelColors(using, false).join(' ')}`,
    ];
    if (ctx.callbackQuery.data === 'tgx-theme') {
        caption.push('');
        caption.push(ctx.i18n('tgx_wallpaper'));
    }
    if (env_1.default.STRIPE_TOKEN) {
        caption.push('');
        caption.push(`If you'd like to help support the bot, please <a href="https://t.me/${ctx.me.username}?start=donate">donate</a>.`);
    }
    await ctx.editMessageMedia({
        type: 'document',
        media: new grammy_1.InputFile(Buffer.from(theme, 'binary'), `${name} by @${ctx.me.username}.${ctx.callbackQuery.data}`),
        caption: caption.join('\n'),
        parse_mode: 'HTML',
    });
    await storage_1.saveTheme(ctx, ctx.msg.message_id, null);
    if (ctx.callbackQuery.data === 'attheme') {
        // TODO: possibly move the preview generator to the svg renderer api
        const preview = await theme_preview_1.default({
            name,
            type: 'attheme',
            theme,
        });
        if (preview) {
            await ctx.replyWithPhoto(new grammy_1.InputFile(preview), {
                caption: 'Preview by @ThemePreviewBot',
                reply_to_message_id: ctx.msg.message_id,
            });
        }
    }
});
requireTheme.on('callback_query', async (ctx) => {
    const themeId = ctx.msg.message_id;
    const { data } = ctx.callbackQuery;
    let color;
    let label;
    switch (data) {
        case 'black':
            color = '#000000';
            label = 'Black';
            break;
        case 'white':
            color = '#ffffff';
            label = 'White';
            break;
        default: {
            const _data = Number(data);
            color = ctx.theme.colors[_data];
            label = (_data + 1).toString();
        }
    }
    if (ctx.theme.using[0]?.color === color) {
        await ctx.answerCallbackQuery({ text: ctx.i18n('cant_reuse_bg') });
        return;
    }
    ctx.theme.using.push({ color, label });
    await storage_1.saveTheme(ctx, themeId, ctx.theme);
    const keyboard = keyboard_1.getKeyboard(ctx, true);
    const { length } = ctx.theme.using;
    if (length < 3) {
        await ctx.editMessageCaption({
            caption: ctx.i18n(`choose_color_${length + 1}`, {
                colors: colors_1.labelColors(ctx.theme.using).join(', '),
            }),
            reply_markup: keyboard,
        });
    }
    else {
        await ctx.editMessageCaption({
            caption: ctx.i18n('type_of_theme'),
            reply_markup: keyboard_1.getTypeKeyboard(ctx),
        });
    }
});
exports.default = composer;
